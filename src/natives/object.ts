 ...
 * @param path - The string path specifying the property to retrieve. Supports dot notation (e.g., 'user.name') and bracket notation (e.g., 'user.posts[0]').
 ...
 * @param path - Whether to include the path in the result. If `true`, the path to the matching value is returned alongside the result.
 * @returns A flat array of matching values or matching values with paths (if `path` is true).
 */
export function queryObject<Ret = unknown, T = unknown, P extends boolean = false>(
  obj: unknown,
 ...
 *
 *   filterObject({ user: { id: 1, profile: { name: 'Ada', age: 36 } } }, (key) => key === 'name' || key === 'profile');
...
/**
 * Recursively filters an object or array tree, preserving the original structure but only keeping items that match the specified key and/or value filters.
 *
 * @template T - The type of the object or array to filter.
 * @param obj - The object or array to filter.
 ...
 *   - `values`: Predicate to test values. If both are provided, both conditions must be satisfied.
 * @returns A new object/array with the same structure, containing only matching items.
 */
...
export function filterObject<T>(
   ...
   */
  function shouldKeepValue(key: string, value: unknown, path: string, parent: unknown): boolean {
   ...
   */
  function shouldKeepKey(key: string, propValue: unknown, path: string, parent: unknown): boolean {
   ...
   */
  function filterArray(arr: readonly unknown[], currentPath: string): unknown[] | undefined {
   ...
   */
  function filterObjectProps(obj: Record<string, unknown>, currentPath: string, parent: unknown): Record<string, unknown> | undefined {
   ...
   */
  function recurse(value: unknown, currentPath: string, parent: unknown): unknown {
 ...
 *
 *   mapObject({ prices: [{ amount: 10 }, { amount: 15 }] }, (key, value) => (key === 'amount' && typeof value === 'number' ? value * 100 : value));
 ...
 *
 *   mergeObject({ user: { name: 'Ada', tags: ['admin'] } }, { user: { name: 'Grace', tags: ['editor'] } });
